...
### A2. step engine
 ...
 - on `/start`, propose parsing the README checklist and present it as a VS Code built-in to-do list (Problems/Tasks or TODO extension) or an in-editor Copilot checklist. Copilot should generate and show a preview of this checklist and ask for explicit approval before creating any persistent session file in `context/`. After approval, the persistent session file may be saved; otherwise, the VS Code built-in to-do is the authoritative work queue.
