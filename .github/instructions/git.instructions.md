...
- CI validation: optionally a repository-level GitHub Actions workflow may validate `context` files and ensure a `context/*-todo.md` exists for PRs that change README or docs. If present, this CI check should be validation-only and must not create files; it should fail the PR if a required `todo` file is not included.
